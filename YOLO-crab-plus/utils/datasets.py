...
class LoadImages_wc:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            imga = cv2.imread(path)  # BGR
            # camera undistort --GuuX made on Canon EOS M100
            ...
            new_camera_matrix, roi = cv2.getOptimalNewCameraMatrix(camera_matrix, dist_coeffs, imga.shape[:2][::-1], 0)
            img0 = cv2.undistort(imga, camera_matrix, dist_coeffs, None, new_camera_matrix)
